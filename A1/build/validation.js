...
function handleSubmit() {
    ...
    const form = document.forms[replyForm.name];
    const submittedForm = {};
    //loop through the salesForm fields and get the actual results from the form
    for (var key in replyForm.fields) {
        if (replyForm.fields.hasOwnProperty(key)) {
            console.log(key + " -> " + replyForm.fields[key]);
            const submission = form[replyForm.fields[key]].value;
...
function handleOriginalFields() {
    ...
    Object.entries(replyForm.fields).forEach(([property,field]) => {
...
function setOriginalFields() {
    ...
    Object.entries(originalErrMsg).forEach(([property,field]) => {
        const fieldID = `#${getErrField(replyForm.fields[property])}`
        const f = document.querySelector(fieldID);
        f.innerHTML = field;
...
function validateForm(submittedForm) {
    ...
    const errors = getErrors(replyForm.validations,submittedForm);
